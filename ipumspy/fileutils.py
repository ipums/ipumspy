...
def xml_opener(ddi_file: FILE_TYPE):
  ...
  if isinstance(ddi_file, Path):
    ...
    if ddi_file.is_dir():
      ...
      possible_files = [filename for filename in ddi_file.iterdir() if filename.suffix == '.xml']
      ...
      with zipfile.ZipFile(ddi_file) as inzip:
        possible_files = [zipinfo for zipinfo in inzip.infolist() if zipinfo.filename.endswith('.xml')]
        if not possible_files:
          raise ValueError(f'{ddi_file} contains no XML files')
        elif len(possible_files) > 1:
          raise ValueError(f'{ddi_file} contains more than one XML file')
