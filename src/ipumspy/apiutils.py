...
class IpumsApiClient(object):
    ...
    def submit_extract(self, collection, request_body):
        extract = ApiRequestWrapper.api_call(
            "post",
            self.base_url,
    ...
    def extract_status(self, collection, extract_number):
        ...
        extract_status = ApiRequestWrapper.api_call(
            "get",
            new_url,
            params={"collection": collection, "version": self.api_version},
            headers={"Authorization": self.api_key},
    ...
    def download_extract(self, collection, extract_number, download_dir=None):
        ...
        if self.extract_status(collection, extract_number) != "completed":
            ...
            for f in files:
                ...
                print(download_path)
                with requests.get(
                    f, stream=True, headers={"Authorization": self.api_key}
                ) as r:
                    r.raise_for_status()
    ...
    def retrieve_previous_extracts(self, collection, N="10"):
        previous_extracts = ApiRequestWrapper.api_call(
            "get",
            self.base_url,
            params={"collection": collection, "limit": N, "version": self.api_version},
            headers={"Authorization": self.api_key},
        )
