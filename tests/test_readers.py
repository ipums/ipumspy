...
def test_can_read_rectangular_dat_gz_chunked(fixtures_path: Path):
  ...
  """
  ddi = readers.read_ipums_ddi(fixtures_path / 'cps_00006.xml')
  data = readers.read_microdata_chunked(ddi, fixtures_path / 'cps_00006.dat.gz', chunksize=1)
  ...
  for df in data:
    ...
    if seen < 5:
      ...
      assert (df['HWTSUPP'].iloc[:5 - seen] == first_hwtsupp[seen:min(len(df) + seen, 5)]).all()
      seen == len(df)
